...
- **啸声抑制**：针对 VoxCPM 长文本合成时高频噪声累积的问题，在音频输出端增加了移动平均低通滤波（窗长 7，边缘填充后 valid 卷积），有效抑制逐渐出现的啸声，同时保留语音清晰度。实现见 `app.py` 中 `_lowpass()` 函数。
- **参考音频建议**：上传的参考音频建议使用**真人录制**的干净语音（5-30 秒），避免使用 AI 生成的音频或高压缩比文件，否则可能引入额外噪声或导致音色克隆效果下降。
- **标点停顿优化**（VoxCPM-0.5B 模式）：自动将句号、感叹号等长停顿标点替换为逗号，减少朗读停顿感，语流更连贯。
- **音色一致性**（VoxCPM2 模式）：音频段落以上传的参考音频为锚定，确保同一主播的每句话音色完全一致。
...
# 指定文本和参考音频
python demo.py --text "你好" --ref-audio voice.wav  # 参考音频克隆音色 
